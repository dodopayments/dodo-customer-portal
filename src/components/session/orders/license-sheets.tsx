...
export const LicenseSheets = ({ isLicenseSheetOpen, setIsLicenseSheetOpen, licenseKeys }: LicenseSheetsProps) => {
                        ...
                        return (
                            <Card key={licenseKey.id} className="p-4">
                                ...
                                <CardContent className="p-0">
                                    <div className="flex items-center justify-between rounded-lg border bg-card px-4 py-3">
                                        <p className="text-sm font-medium text-text-secondary">
                                            {isVisible ? (
                                                <span className="font-mono text-text-primary">{licenseKey.key}</span>
                                            ) : (
                                                <span className="italic">Click to view license key</span>
                                            )}
                                        </p>
                                        <Button
                                            variant="ghost"
                                ...
                                <CardFooter className="flex flex-row justify-between p-0 pt-3">
                                    <Badge variant={getBadge(licenseKey.status, false, true).color as BadgeVariant}>
                                        {getBadge(licenseKey.status, false, true).message}
                                    </Badge>
                                    <p className="text-sm text-text-secondary">Expires {parseIsoDate(licenseKey.expires_at)}</p>
                                </CardFooter>
                            </Card>
                        );
                    })}
