...
export const Product = ({ payment_id, subscription_id, product }: { payment_id: string, subscription_id: string | null, product: ProductCartItem }) => {
    ...
    const fetchDigitalProducts = async () => {
        try {
            ...
            const response = await fetch(`${api_url}/customer-portal/payments/${payment_id}/digital-product-deliverables`, {
                headers: {
    ...
    async function getSubscription() {
        try {
            const token = await getToken();
            const response = await fetch(`${api_url}/customer-portal/subscriptions/${subscription_id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
        ...
        <Card className="p-6">
            <CardContent className="flex flex-row justify-between px-0 gap-2">
                <div className="flex flex-row items-center gap-2">
                    <Image src={product.product_image} alt={product.product_name} width={56} height={56} className="rounded-lg flex-none aspect-square object-cover" />
                    <div className="flex flex-col gap-2">
                        <CardTitle className="font-['Hanken_Grotesk'] font-semibold text-base leading-5 flex-none">{product.product_name}</CardTitle>
                        <CardDescription className="font-['Inter'] font-normal text-sm leading-[21px] text-text-secondary self-stretch">{product.product_description}</CardDescription>
                    </div>
                </div>
                <div className="font-['Hanken_Grotesk'] font-semibold text-base leading-5 flex-none">{formatCurrency(decodeCurrency(product.price, product.currency as CurrencyCode), product.currency as CurrencyCode)}</div>
            </CardContent>
            <Separator className="my-4" />
            <CardFooter className="flex flex-row justify-between p-0">
                {!product.is_subscription && product.quantity > 0 && <div className="flex flex-row gap-2">
                    <p className="text-sm text-text-secondary">QTY: {product.quantity}</p>
                </div>}
                {subscription && (
                    <div className="flex flex-row gap-2">
                        <Badge variant="default" dot={false} className="rounded-sm border-sm">
                            renews on {new Date(subscription.next_billing_date).toLocaleDateString("en-GB", {
                                day: "numeric",
                                month: "short",
                                year: "2-digit",
                            })}
                        ...
                        <Badge variant="default" dot={false} className="rounded-sm border-sm">
                            valid until {new Date(subscription.next_billing_date).toLocaleDateString("en-GB", {
                                day: "numeric",
                                month: "short",
                                year: "2-digit",
                            })}
                ...
                <div className="flex flex-row gap-2">
                    ...
                    {product.is_subscription && (
                        <Button variant="secondary" className="w-full" onClick={() => router.push(`/session/subscriptions/${subscription_id}`)}>
                            Manage Subscription
                        </Button>
                    )}
                </div>
