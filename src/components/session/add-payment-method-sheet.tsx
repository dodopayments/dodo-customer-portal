...
import { formatCardNumber, formatExpiryDate, unformatCardNumber, parseExpiryDate } from "@/lib/payment-helper"


export function AddPaymentMethodSheet() {
    const [open, setOpen] = useState(false);
    const [selected, setSelected] = useState<"card" | "gpay" | "applepay" | "affirm" | "klarna">("card");
    ...
    const handleExpiryDateKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key === "Backspace" && expiryDate.length === 3 && expiryDate.endsWith("/")) {
    ...
    const handleAddCard = () => {
        ...
        console.log(cardNumberDigits, expiryParsed?.month, expiryParsed?.year, securityCode);
    }

    return (
        <Sheet open={open} onOpenChange={setOpen}>
            <Button variant="secondary" className="my-auto" onClick={() => setOpen(true)}>
                Add payment method
            </Button>
            <SheetContent className="flex flex-col gap-4">
                <SheetHeader>
                    <SheetTitle className="text-left font-['Hanken_Grotesk'] font-semibold text-base leading-tight tracking-normal">Add payment method</SheetTitle>
                </SheetHeader>
                <Separator className="my-3" />
                <div className="flex flex-col gap-4">
                    <div className="flex flex-row gap-2 w-full">
                        <Button className="w-full"><ApplePay size={30} className="my-auto" invert={false} /></Button>
                        <Button variant="secondary" className="w-full"><GPay size={30} className="my-auto" /></Button>
                    </div>
                    ...
                    <div className="flex flex-row gap-2 w-full">
                        ...
                        >
                            <Image src="/payment-methods/affirm.svg" className="my-auto" alt="Affirm" width={30} height={30} />
                        ...
                        >
                            <Image src="/payment-methods/klarna.jpg" className="my-auto" alt="Klarna" width={16} height={16} />
                    ...
                    {selected === "card" && (
                        <div className="flex flex-col gap-4">
                            ...
                            <p className="text-sm text-text-secondary">
                                By providing your card information, you allow Lummi to charge your card for future payments in accordance with their terms.
                            </p>
                            <Separator className="my-3" />
                        </div>
                    )}
                    <Button className="w-full" onClick={handleAddCard}>Add</Button>
