        ...
        <div className="flex flex-row gap-4 p-2 w-full">
          <div className="border border-border-secondary rounded-md p-2">
            {paymentMethod.brand === "mastercard" && (
              <Mastercard size={30} className="my-auto" />
            )}
            {paymentMethod.brand === "google" && (
              <GPay size={30} className="my-auto" />
            )}
            {paymentMethod.brand === "apple" && (
              <ApplePay size={30} className="my-auto" />
            )}
          </div>
          <p className="text-text-primary text-base font-medium leading-none my-auto">
            {paymentMethod.name}
          </p>
          {paymentMethod.type === "card" && (
            <div className="flex flex-row gap-2">
              <p className="text-text-secondary my-auto">••••</p>
              <p className="text-text-secondary my-auto">
                {paymentMethod.last4}
              </p>
            </div>
          )}
